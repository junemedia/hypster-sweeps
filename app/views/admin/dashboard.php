...
?><!-- htmlmin:ignore --><?= $li_open ?>
<? if ($c): ?>
    <a href="/admin/prize/<?= $c['prize_id'] ?>#<?= $c['date'] ?>">
        <img src="<?= $c['prize_img1']?>"/>
        <h4><?= $c['prize_title'] ?></h4>
    </a>
    <h5>$<?= $c['prize_value'] . ' ' . ($c['prize_type'] == 'giftcard' ? 'Gift Card' : 'Gift Card or Prize') ?></h5>
<? else: ?>
    <a href="/admin/prize/0">
        <img/>
        <h4>Add Prize</h4>
    </a>
    <h5>–</h5>
<? endif; ?>
<? if (@$c['user_id']): ?>
    <a href="mailto:<?= $c['user_email'] ?>"><?= $c['user_firstname'] . ' ' . $c['user_lastname'] ?></a>
<? else: ?>
    <a>–</a>
<? endif; ?>
    <em><?= $dom ?></em>
